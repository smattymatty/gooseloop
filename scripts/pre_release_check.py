...
Run before ``uv publish``. Exits 0 on match, 1 on mismatch. The release rule:
a version ships only when the CHANGELOG names it with a date, so the operator
has written down what the release contains before it exists.
"""
...
# Matches "## [0.1.0] - 2026-07-12" with version captured. Skips
