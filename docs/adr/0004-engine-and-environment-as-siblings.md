...
ADR 0001 introduced the Engine as a pluggable component that owns "what a particular pipeline does." In practice, the first engine (`CustomerAcquisitionEngine`) ended up owning two distinct kinds of state:
...
2. **Project-specific data** — the origin project's config paths, the digest builder that knows what a "prospect" file looks like, the journal location, the project context manifest. None of this is intrinsic to "customer-acquisition pipelines"; it's intrinsic to one specific project.

These two kinds of state were conflated. A hypothetical second user (call them BetaCo) wanting to run the customer-acquisition pipeline against their own prospects would have to fork the engine entirely, because the origin project's specific paths, prospect schema, and journal location are baked into engine code.

A 2026-05-31 design-review session crystallised the missing primitive: **what the pipeline does** (engine, verbs) is a different concern from **what the pipeline has access to** (project data, nouns). The Pass 2 context-prepend mechanism (recipes declare inputs, engine pre-renders to literal text) made this gap acute — recipes need access to project data, but the engine isn't a clean place to host it if we want pluggable projects.
...
```python
env = AcmeEnvironment.from_toml("environment.toml")
...
- Project-specific configuration in `environment.toml` (renamed from the engine's own config)
...
- `environment.toml` is honest about what it configures. Previously the engine-named config file named the *engine* but configured the *environment*.
...
- Migration cost: existing `CustomerAcquisitionEngine` code that owns paths and the digest builder has to move to the concrete environment. Tests, recipes, and runner.py all need touching. One coherent PR; not landed yet at the time of this ADR.

## Migration plan

The bash-migration for `pipeline-review.yaml` (the original 2026-06-01 design review topic) is *blocked on* this Environment landing. Doing the recipe migration first with workarounds, then re-migrating to `env_method:`, was rejected as throwaway work.

Order of changes:

1. Create the concrete environment class with paths + loaders. Read from renamed `environment.toml`.
...
5. Move `build_digest`, `journal_path`, the prospect loader, and the questions lister into the concrete environment. Engine calls into env for these.
...
- **Tier 4 scope: Environment also owns scoring weights.** Maximum project autonomy. Rejected (for now) because the line between "scoring algorithm" (engine) and "scoring weights" (environment) is real but the current code has one prospect-scoring system that works for the origin project; abstracting weights pre-emptively is YAGNI. Revisit if a second environment exists.
- **Directory reorganisation (`environments/acme/`, `engines/customer_acquisition/`)** in the same change. Future-proofs the OSS layout but bigger restructure; touches every import path and test fixture. Deferred to a follow-up.
