...
`runner.py` today hardcodes the origin customer-pipeline shape: validate → review → snapshot → branch → periodic → summary. We plan to extract `GooseLooper` as a generic OSS execution shell, with the origin customer pipeline becoming the first **Engine** and future engines (e.g. a security monitor) plugging in alongside.

The crux question: who owns the phase order — the Looper or the Engine? A security-monitoring engine has nothing in common with a customer-pipeline engine at the phase level. Baking the origin shape into the Looper would force every future engine into the same mold.
...
- New engines (a security monitor, third-party) are additive — they ship their own `pipeline()` and recipes, no Looper changes needed.
...
- **Looper hardcodes canonical shape (review → branches → periodic → summary), engine fills hooks.** Simpler, lower learning curve, but bakes the origin pipeline into the OSS core. Rejected because the OSS goal makes origin-shaped abstractions a liability.
